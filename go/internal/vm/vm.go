...
type VM struct {
	...
	// Async state (stubs for out-of-scope opcodes)
	nextTaskID    int
	nextGroupID   int
	nextChannelID int
	nextTVarID    int
...
func (vm *VM) dispatch(opcode byte, code []byte) error {
	...
	case compiler.OpDEBUG:
		...
		vm.Stdout = append(vm.Stdout, fmt.Sprintf(`{"debug":[%s]}`, strings.Join(parts, ",")))
	...
	case compiler.OpTVAR_NEW:
		initial, _ := vm.pop()
		tv := &TVar{ID: vm.nextTVarID, Version: 0, Val: initial, Occupied: true, HandleCount: 1}
		vm.nextTVarID++
	...
	case compiler.OpREF_NEW:
		initial, _ := vm.pop()
		ref := &RefCell{ID: vm.nextRefID, Val: initial, HandleCount: 1}
		vm.nextRefID++
		vm.push(ValRef(ref))
	...
	case compiler.OpITER_NEW:
		...
		iter := &IteratorState{
			ID: vm.nextIterID, GeneratorFn: generatorFn, CleanupFn: cleanupFn,
		}
		vm.nextIterID++
...
func (vm *VM) builtinIterOf() error {
	...
	copy(src, list)
	iter := &IteratorState{
		ID: vm.nextIterID, GeneratorFn: ValUnit(), CleanupFn: ValUnit(),
		Source: src,
	}
	vm.nextIterID++
	vm.push(ValIter(iter))
...
func (vm *VM) opTaskSpawn() error {
	...
	vm.tasks[taskID] = task
	if groupID != 0 {
		if g, ok := vm.taskGroups[groupID]; ok {
...
func (vm *VM) opTaskAwait() error {
	...
	if task.status == "suspended" {
		if err := vm.runTaskToCompletion(task); err != nil {
			return err
		}
	}
	if task.status == "completed" {
		...
		if !vm.doRaise(task.errMsg) {
			return vm.trap("E014", fmt.Sprintf("child task failed: %s", task.errMsg))
		}
	}
	return nil
}

func (vm *VM) runTaskToCompletion(task *taskState) error {
	savedDataStack := make([]Value, len(vm.dataStack))
	copy(savedDataStack, vm.dataStack)
	savedCallStack := make([]CallFrame, len(vm.callStack))
	copy(savedCallStack, vm.callStack)
	savedHandlerStack := make([]HandlerFrame, len(vm.handlerStack))
	copy(savedHandlerStack, vm.handlerStack)
	savedIP := vm.ip
	savedWord := vm.currentWord
	savedTaskID := vm.currentTaskID
	savedHalted := vm.halted
	savedTopFrame := vm.topFrame

	vm.dataStack = append([]Value{}, task.dataStack...)
	vm.callStack = append([]CallFrame{}, task.callStack...)
	vm.handlerStack = append([]HandlerFrame{}, task.handlerStack...)
	vm.ip = task.ip
	vm.currentWord = task.currentWord
	vm.currentTaskID = task.id
	vm.halted = false
	vm.topFrame = task.topFrame
	task.status = "running"
	...
	vm.dataStack = savedDataStack
	vm.callStack = savedCallStack
	vm.handlerStack = savedHandlerStack
	vm.ip = savedIP
	vm.currentWord = savedWord
	vm.currentTaskID = savedTaskID
	vm.halted = savedHalted
	vm.topFrame = savedTopFrame
	return nil
}
...
func (vm *VM) opTaskGroupExit(code []byte) error {
	...
	groupID := vm.activeGroups[len(vm.activeGroups)-1]
	vm.activeGroups = vm.activeGroups[:len(vm.activeGroups)-1]
	group, ok := vm.taskGroups[groupID]
	if !ok {
		return nil
	}
	// Run children to completion
	for childID := range group.childTaskIDs {
		child, ok := vm.tasks[childID]
		if ok && child.status == "suspended" {
			vm.runTaskToCompletion(child)
		}
	}
	// Check for failures
...
func (vm *VM) opChanNew() error {
	...
	ch := &Channel{ID: vm.nextChannelID, Capacity: cap, SenderOpen: true, ReceiverOpen: true}
	vm.nextChannelID++
...
func (vm *VM) opChanSend(code []byte) error {
	...
	if !ch.ReceiverOpen || !ch.SenderOpen {
...
func (vm *VM) opChanRecv(code []byte) error {
	...
	if len(ch.Buffer) > 0 {
		...
		vm.push(v)
	} else if !ch.SenderOpen {
		if !vm.doRaise("channel closed") {
			return vm.trap("E012", "CHAN_RECV: channel is closed and empty")
		}
	} else {
		if !vm.doRaise("channel empty") {
			return vm.trap("E012", "CHAN_RECV: channel is empty")
		}
	}
	return nil
}
...
func (vm *VM) opChanClose() error {
	...
	if endVal.Heap.Kind == KindSender {
		endVal.Heap.Channel.SenderOpen = false
	} else if endVal.Heap.Kind == KindReceiver {
		endVal.Heap.Channel.ReceiverOpen = false
	} else {
		return vm.trap("E002", "CHAN_CLOSE: expected Sender or Receiver")
	}
	vm.push(ValUnit())
...
func (vm *VM) opSelectWait(code []byte) error {
	...
	for _, arm := range arms {
		...
		if src.Tag == TagHEAP && src.Heap.Kind == KindReceiver {
			...
			task, ok := vm.tasks[src.Heap.TaskID]
			if ok {
				if task.status == "suspended" {
					vm.runTaskToCompletion(task)
				}
				if task.status == "completed" {
					v := ValUnit()
					if task.result != nil {
