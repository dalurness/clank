...
func registerBuiltins(env *typeEnv) {
	...
	registerPolyBuiltin(env, "concat", 1, func(v []Type) Type { return NewTFn(v[0], NewTFn(v[0], v[0])) })
	...
	env.set("dt.fmt", NewTFn(TAny, NewTFn(TStr, TStr)))
	env.set("dt.add", NewTFn(TAny, NewTFn(TInt, TAny)))
	env.set("dt.sub", NewTFn(TAny, NewTFn(TAny, TInt)))
	env.set("dt.tz", NewTFn(TAny, NewTFn(TStr, TAny)))
	env.set("dt.iso", NewTFn(TAny, TStr))
	env.set("dt.ms", NewTFn(TInt, TInt))
	env.set("dt.sec", NewTFn(TInt, TInt))
	env.set("dt.min", NewTFn(TInt, TInt))
	env.set("dt.hr", NewTFn(TInt, TInt))
	env.set("dt.day", NewTFn(TInt, TInt))

	// CSV (std.csv)
	tStrList := TList{Element: TStr}
	...
	env.set("cli.get", NewTFn(TAny, NewTFn(TStr, TAny)))
	env.set("cli.flag", NewTFn(TAny, NewTFn(TStr, TBool)))
	env.set("cli.pos", NewTFn(TAny, NewTFn(TInt, TAny)))

	// Iterator combinators
	env.set("iter.of", NewTFn(TAny, TAny))
	env.set("iter.range", NewTFn(TInt, NewTFn(TInt, TAny)))
	env.set("iter.collect", NewTFn(TAny, TAny))
	env.set("iter.map", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.filter", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.take", NewTFn(TAny, NewTFn(TInt, TAny)))
	env.set("iter.drop", NewTFn(TAny, NewTFn(TInt, TAny)))
	env.set("iter.fold", NewTFn(TAny, NewTFn(TAny, NewTFn(TAny, TAny))))
	env.set("iter.count", NewTFn(TAny, TInt))
	env.set("iter.sum", NewTFn(TAny, TInt))
	env.set("iter.any", NewTFn(TAny, NewTFn(TAny, TBool)))
	env.set("iter.all", NewTFn(TAny, NewTFn(TAny, TBool)))
	env.set("iter.find", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.each", NewTFn(TAny, NewTFn(TAny, TUnit)))
	env.set("iter.drain", NewTFn(TAny, TUnit))
	env.set("iter.enumerate", NewTFn(TAny, TAny))
	env.set("iter.chain", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.zip", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.take-while", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.drop-while", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.flatmap", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.first", NewTFn(TAny, TAny))
	env.set("iter.last", NewTFn(TAny, TAny))
	env.set("iter.join", NewTFn(TAny, NewTFn(TStr, TStr)))
	env.set("iter.repeat", NewTFn(TAny, TAny))
	env.set("iter.once", NewTFn(TAny, TAny))
	env.set("iter.empty", NewTFn(TUnit, TAny))
	env.set("iter.unfold", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.scan", NewTFn(TAny, NewTFn(TAny, NewTFn(TAny, TAny))))
	env.set("iter.dedup", NewTFn(TAny, TAny))
	env.set("iter.chunk", NewTFn(TAny, NewTFn(TInt, TAny)))
	env.set("iter.window", NewTFn(TAny, NewTFn(TInt, TAny)))
	env.set("iter.intersperse", NewTFn(TAny, NewTFn(TAny, TAny)))
	env.set("iter.cycle", NewTFn(TAny, TAny))
	env.set("iter.nth", NewTFn(TAny, NewTFn(TInt, TAny)))
	env.set("iter.min", NewTFn(TAny, TAny))
	env.set("iter.max", NewTFn(TAny, TAny))
	env.set("iter.generate", NewTFn(TAny, TAny))

	// Collections (std.col)
	registerPolyBuiltin(env, "col.rev", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, TList{Element: v[0]}) })
	registerPolyBuiltin(env, "col.sort", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, TList{Element: v[0]}) })
	registerPolyBuiltin(env, "col.sortby", 1, func(v []Type) Type {
		return NewTFn(TList{Element: v[0]}, NewTFn(NewTFn(v[0], NewTFn(v[0], TInt)), TList{Element: v[0]}))
	})
	registerPolyBuiltin(env, "col.uniq", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, TList{Element: v[0]}) })
	registerPolyBuiltin(env, "col.zip", 2, func(v []Type) Type {
		return NewTFn(TList{Element: v[0]}, NewTFn(TList{Element: v[1]}, TList{Element: TTuple{Elements: []Type{v[0], v[1]}}}))
	})
	registerPolyBuiltin(env, "col.unzip", 2, func(v []Type) Type {
		return NewTFn(TList{Element: TTuple{Elements: []Type{v[0], v[1]}}}, TTuple{Elements: []Type{TList{Element: v[0]}, TList{Element: v[1]}}})
	})
	registerPolyBuiltin(env, "col.flat", 1, func(v []Type) Type { return NewTFn(TList{Element: TList{Element: v[0]}}, TList{Element: v[0]}) })
	registerPolyBuiltin(env, "col.flatmap", 2, func(v []Type) Type {
		return NewTFn(TList{Element: v[0]}, NewTFn(NewTFn(v[0], TList{Element: v[1]}), TList{Element: v[1]}))
	})
	registerPolyBuiltin(env, "col.take", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, NewTFn(TInt, TList{Element: v[0]})) })
	registerPolyBuiltin(env, "col.drop", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, NewTFn(TInt, TList{Element: v[0]})) })
	registerPolyBuiltin(env, "col.nth", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, NewTFn(TInt, v[0])) })
	registerPolyBuiltin(env, "col.find", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, NewTFn(NewTFn(v[0], TBool), v[0])) })
	registerPolyBuiltin(env, "col.any", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, NewTFn(NewTFn(v[0], TBool), TBool)) })
	registerPolyBuiltin(env, "col.all", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, NewTFn(NewTFn(v[0], TBool), TBool)) })
	registerPolyBuiltin(env, "col.count", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, NewTFn(NewTFn(v[0], TBool), TInt)) })
	...
	env.set("col.prod", NewTFn(TList{Element: TInt}, TInt))
	registerPolyBuiltin(env, "col.min", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, v[0]) })
	registerPolyBuiltin(env, "col.max", 1, func(v []Type) Type { return NewTFn(TList{Element: v[0]}, v[0]) })
	registerPolyBuiltin(env, "col.group", 2, func(v []Type) Type {
		return NewTFn(TList{Element: v[0]}, NewTFn(NewTFn(v[0], v[1]), TList{Element: TTuple{Elements: []Type{v[1], TList{Element: v[0]}}}}))
	})
	registerPolyBuiltin(env, "col.scan", 2, func(v []Type) Type {
		return NewTFn(TList{Element: v[0]}, NewTFn(v[1], NewTFn(NewTFn(v[1], NewTFn(v[0], v[1])), TList{Element: v[1]})))
	})
}
...
func typeCheckImpl(program *ast.Program, resolver ModuleTypeResolver, aliasResolver ModuleEffectAliasResolver) []TypeError {
	...
	for _, tl := range program.TopLevels {
		...
		case ast.TopUseDecl:
			...
			// Determine qualifier for qualified imports (e.g., "summary" from use reports.summary)
			qualifier := ""
			if d.Qualified {
				qualifier = d.Qualifier
				if qualifier == "" {
					qualifier = d.Path[len(d.Path)-1]
				}
			}
			...
			for _, imp := range d.Imports {
				...
				if qualifier != "" {
					dottedName := qualifier + "." + imp.Name
					env.set(dottedName, resolvedType)
					if moduleTypes != nil {
						env.setScheme(dottedName, TypeScheme{})
					}
				}
