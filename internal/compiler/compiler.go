...
var builtinOps = map[string][]byte{
	...
	"not":    {OpNOT},
	"str.cat": {OpSTR_CAT}, "concat": {OpCONCAT}, "show": {OpTO_STR}, "print": {OpIO_PRINT},
...
var vmBuiltins = map[string]int{
	...
	"http.patch": 124, "http.req": 125, "http.hdr": 126, "http.json": 127, "http.ok?": 128, "http.set-timeout": 129,
...
func (c *Compiler) firstPass(tl ast.TopLevel) {
	...
	case ast.TopUseDecl:
		// Determine qualifier for qualified imports
		qualifier := ""
		if t.Qualified {
			qualifier = t.Qualifier
			if qualifier == "" {
				qualifier = t.Path[len(t.Path)-1]
			}
		}
		...
		for _, imp := range t.Imports {
			...
			// For qualified imports, also register qualifier.name → same word ID
			if qualifier != "" {
				dottedName := qualifier + "." + imp.Name
				if origID, ok := c.wordIDs[imp.Name]; ok {
					c.wordIDs[dottedName] = origID
				}
				if vi, ok := c.variantInfos[imp.Name]; ok {
					c.variantInfos[dottedName] = vi
				}
			}
		}
	}
